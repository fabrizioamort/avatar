...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --cpu-throttling \
  --port 8080 \
  --set-env-vars "^@^MODEL=${MODEL}@OWNER_NAME=${OWNER_NAME}@ENVIRONMENT=production@COOKIE_SECURE=1@TRUST_PROXY_HEADERS=1@GOOGLE_CLOUD_PROJECT=${PROJECT_ID}@FIRESTORE_DATABASE=(default)" \
